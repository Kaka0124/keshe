...
int MachineState::requiredGpuCount(const Job &job) const {
    // Minimum GPUs needed to satisfy both count and memory requirements
    int gpu_for_memory = (job.gpu_memory + spec.gpu_memory - 1) / spec.gpu_memory;
    return std::max(job.min_gpu, gpu_for_memory);
}

bool MachineState::canEverRun(const Job &job) const {
    int gpu_needed = requiredGpuCount(job);
    return gpu_needed <= spec.gpu_count &&
           job.cpu_cores <= spec.cpu_cores &&
           job.memory <= spec.memory &&
           static_cast<long long>(job.gpu_memory) <= static_cast<long long>(gpu_needed) * spec.gpu_memory;
}

bool MachineState::canStart(const Job &job) const {
    int gpu_needed = requiredGpuCount(job);
    return canStart(job, gpu_needed);
}
...
bool MachineState::canStart(const Job &job, int gpu_used) const {
    return gpu_used <= remaining_gpu &&
           ...
           static_cast<long long>(job.gpu_memory) <= static_cast<long long>(gpu_used) * spec.gpu_memory;
}

pair<ScheduleRecord, RunningJob> MachineState::startJob(
    const Job &job, long long current_time
) {
    return startJob(job, current_time, requiredGpuCount(job));
}

pair<ScheduleRecord, RunningJob> MachineState::startJob(
    const Job &job, long long current_time, int gpu_used
) {
    ...
    gpu_memory_used += job.gpu_memory;
